...
Indeed, it soon turned out that by far most of the development time of MiMA went into the mixed layer rather than the new radiation scheme: Switching the radiation and adding all necessary inputs/outputs and parameters maybe took a month or two, whereas the adjusting of the mixed layer, including adding new features to make it more flexible and/or more realistic took several months to a year. In fact, the mixed layer is still a focus of further development two years after first publishing the model.

Depending on the depth of the mixed layer, the model has to be spun up longer (deep) or shorter (shallow) until it converges. For the simulations shown below, we ran MiMA for a spinup of ten years for a mixed layer of 10m before averaging, whereas the 100m mixed layer depth required 30 years to converge. We define "convergence" here as the time it takes for the seasonal cycle of temperature to not show any temporal trend. The tropical deep mixed layer ocean was the slowest to converge. Note that these convergence times should always be checked as different setups will most likely require a varying number of years to converge. After spinup, we show averages over ten years. The runs were done with a T42 resultion and 20 vertical levels (without any sponge layer at the model top).
...
Other _a priori_ missing component of MiMA are sea ice, snow cover and clouds. This is not a major shortcoming since ocean dynamics, biogeochemistry and others are willingly left out as well to keep the model simple. One common characteristic of sea ice, snow cover and clouds is that they are white, and thus they all have a certain albedo effect on the atmosphere. At least for sea ice, the albedo can be adjusted rather easily, at least in a static sense. Fig. 6 shows examples of adding higher albedo south of 65&deg;S to mimic the effect of Antarctica (yellow line) and different implemented choices for meridional albedo profiles.
...
To add comments, please [klick here to send an email to the editor](mailto:editor.iceberg@gmail.com?Subject=Comments%20on%20M_Jucker_201907), who will add them to this document. 
